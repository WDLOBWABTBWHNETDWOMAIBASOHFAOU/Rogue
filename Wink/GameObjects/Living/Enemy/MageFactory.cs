...
namespace Wink
{
    class MageFactory : EnemyFactory
    {
        public MageFactory(int floorNumber) : base(floorNumber)
        {
        }

        public override Enemy CreateEnemy()
        {
            Enemy enemy = new Enemy(floorNumber, 0, "Enemy : Mage");
            ...
            int eLvl = ELvl;
            enemy.SetStats(eLvl, 1 + (eLvl / 2), 1 + (eLvl / 2), 1 + (eLvl / 2), 3 + (eLvl), 3 + (eLvl), 1 + (eLvl / 2), 20 + eLvl * 3, 2, 2);
            return enemy;
        }
    }
}
